		...
		if user:
			...
			if request.POST.get('json'):
				...
				if req_data.get('image_seed'):
					if not user_settings_manager.update_image_seed(req_data['image_seed']):
						return JsonResponse({"message": f"Error: Invalid image seed!"}, status=409)
				fields_to_update = {
			...
			if request.FILES:
				if not user_settings_manager.update_image(request.POST, request.FILES):
					return JsonResponse({"message": f"Error: Invalid input image! Image must be in format jpeg/png!"}, status=409)
